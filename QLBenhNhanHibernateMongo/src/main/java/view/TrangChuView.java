...
public class TrangChuView extends JFrame implements ActionListener, MouseListener {
	...
	private HoSoBenhAn hoSoBenhAnTotal = new HoSoBenhAn();
	...
	public TrangChuView() {
		...
		btnQuanLyPhieuKham.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(obj.equals(btnQuanLyPhieuKham)) {
					...
					panel_2.removeAll();
					panel_2.add(new QuanLyPhieuKhamView(hoSoBenhAnTotal));
					panel_2.repaint();
					panel_2.revalidate();
				}
			}
		...
		btnQuanLyToaThuoc.addActionListener(new java.awt.event.ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(obj.equals(btnQuanLyToaThuoc)) {
					panel_2.removeAll();
					panel_2.add(new QuanLyToaThuocView(hoSoBenhAnTotal));
					panel_2.repaint();
					panel_2.revalidate();
				}
			}});
		...
		btnThem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				hoSoBenhAnTotal = hoSoBenhAnService.findByPhoneNumber(splits[0]);
				System.out.println(hoSoBenhAnTotal.toString());
...
//		});
	...
	public void docLenTBL() {
		for (HoSoBenhAn benh : hoSoBenhAnService.findAll()) {
			Object[] ob = { benh.getHoTen(), benh.getTuoi(), benh.getDienThoai(), benh.getDiaChi(),
					new SimpleDateFormat(CrudStatus.DATETIME_FORMAT).format(benh.getNgayLap()) };
			dtf.addRow(ob);
		}
	}
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (o.equals(table)) {
			...
			HoSoBenhAn ba = hoSoBenhAnService.findByPhoneNumber(splits[0]);
			...
			Object[] ob = { ba.getHoTen(), ba.getTuoi(), ba.getDienThoai(), ba.getDiaChi(),
					new SimpleDateFormat(CrudStatus.DATETIME_FORMAT).format(ba.getNgayLap()) };
			dtf.addRow(ob);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(ob.equals(btnLamMoi)) {
			...
			String id = hoSoBenhAnService.findByPhoneNumber(std).getId();
			
			if(hoSoBenhAnService.deleteById(id) == "success") {
			...
			String id = hoSoBenhAnService.findByPhoneNumber(std).getId();
			Date date = hoSoBenhAnService.findByPhoneNumber(std).getNgayLap();
			...
			HoSoBenhAn hoSoBenhAn = new HoSoBenhAn(id, hoten, tuoi, diaChi, sdt, date);
			
			
//			HoSoBenhAn benhAn = new HoSoBenhAn(hoten, tuoi, diaChi, sdt);
			
			if(hoSoBenhAnService.updateHoSoBenhAn(hoSoBenhAn)) {
